...
> 上下文:`docs/research/2026-09-08-windows-handoff.md` + `outputs/eval_history.md` 全部 12 节
...
1. **官方评测是唯一裁判**:所有实验(含失败)如实追加 `outputs/eval_history.md`
...
共享底座:15fps 帧缓存(`temp/work/<video>/frames15/`,现有)、特征缓存(新 `temp/features/<video>/`)、素材登记表(`ground_truth/videos.jsonl`)、自主标注管线(`tools/autolabel/`)、统一评测。
...
- **素材登记表** `ground_truth/videos.jsonl`:每段登记 id/来源/机位/场馆/许可/域标签,驱动分组采样与留一验证
...
4. 失败也入 `outputs/eval_history.md`,注明根因与去向
